...
def ldm_process(
    ...
    output_queue,
    ensemble_size=32,
    weights_fn="/home/dupf/ldcast/ldcast-master/scripts/lightning_logs/version_0/checkpoints/epoch=86-val_loss_ema=0.0454.ckpt",
    num_diffusion_iters=50,
):
    idx = 0
    (ldm, trainer) = train_genforecast.setup_model()
    device = "cuda:2"  # Specify the GPU device
    ldm.to(device=device)
...
def create_evaluation_ensemble(
    out_dir="../results/eval_ensembles/TCIR/",
    ...
    print(f'Queue size before join: {compute_queue.qsize()}')
