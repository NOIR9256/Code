...
class Diffusion:
    def __init__(self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, img_size=256, device="cuda"):
        ...
        # t = torch.arange(noise_steps + 1, dtype=torch.float32, device=device)
        # # 余弦公式计算alpha_bar_t (Improved DDPM)
        # alpha_bar_t = torch.cos((t / noise_steps + 0.008) / (1 + 0.008) * math.pi / 2) ** 2
        # alpha_bar_t = alpha_bar_t / alpha_bar_t[0]
        # alpha_t = alpha_bar_t[1:] / alpha_bar_t[:-1]
        # beta = 1 - alpha_t
        # self.beta = torch.clip(beta, 0.0001, 0.9999)
        #
        # self.alpha = 1. - self.beta
        # self.alpha_hat = torch.cumprod(self.alpha, dim=0)
