...
def plot_crps(
    ...
    crps[model] = {}
    fn = f"../results/crps/{crps_name}-pm-{model}.nc"
    ...
    for scale in scales:
        ...
        max_t = max(max_t, t[-1])
        ax.plot(t, score, color=color, linestyle=linestyle,label=model_labels)

    if add_legend:
        plt.legend()
    if add_xlabel:
        plt.xlabel("Lead time [hour]", fontsize=12)
    if add_ylabel:
        plt.ylabel(
...
def plot_rank_distribution(
    ...
    # for model in models:
    model = models
    fn = f"../results/ranks/ranks-pm-{model}.nc"
    with netCDF4.Dataset(fn, 'r') as ds:
        for scale in scales:
...
def plot_fss(
    ...
    with concurrent.futures.ProcessPoolExecutor(N_threads) as executor:
        ...
        fss_scale[models] = {}
        for scale in scales:
            fss_scale[models][scale] = executor.submit(
                load_fss, models, scale, use_timesteps, crop_box
            )

        # for model in models:
        # model = models
        for scale in scales:
            fss_scale[models][scale] = fss_scale[models][scale].result()
    ...
    scales = sorted(fss_scale[models])
    fss_for_model = [fss_scale[models][s] for s in scales]

    model_parts = models.split("-")
    if models.startswith("pm-"):
