...
def load_nc_file(file_path):
    with nc.Dataset(file_path, 'r') as data:
        ...
        times = data.variables['patch_times'][:]
    return patches, times

def load_split(files):
    patches_list = []
    times_list = []
    for file in files:
        patches, times = load_nc_file(file)
...
# 使用示例
data_dir = '/home/dupf/ldcast/ldcast-master/data/TCIR_data'  # Adjust this path to your data directory
