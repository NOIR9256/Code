...
from ldcast.features.utils import average_pool

# 获取patch，也就是从大图中选取32x32的小图，但不是最终送进去的patch小块
def patch_locations(
    time_range,
    patch_box,
    patch_shape=(32,32),
...
#         file_var = fn.split("_")[1]
#         if file_var == var:
#             fn_path = os.path.join(patch_dir, fn)
#             # 将任务添加到当前批次
#             batch_jobs.append(dask.delayed(load_patches)(fn_path))
#
#             # 检查是否达到批次大小，如果是，则提交当前批次
#             if len(batch_jobs) == batch_size:
#                 results = dask.compute(*batch_jobs, scheduler="processes")
#                 # 将当前批次的结果添加到累积器
...
#         all_results.extend(results)
    ...
    # for k in keys:
    #     patch_data[k] = np.concatenate(
    #         [fd[k] for fd in all_results],
    #         axis=0
    #     )
    # patch_data["zero_value"] = all_results[0]["zero_value"]
    # if "scale" in all_results[0]:
    #     patch_data["scale"] = all_results[0]["scale"]
    #
    # return patch_data
