...
# def scale_array(in_arr, out_arr, scale):
#     in_arr = in_arr.ravel()
#     normalized_matrix = in_arr - in_arr.min()
#     normalized_matrix /= normalized_matrix.max()
#     scaled_matrix = np.round(normalized_matrix * 255)
#     final_matrix = scaled_matrix.astype(np.uint8)
#     out_arr = out_arr.ravel()
#     for i in prange(in_arr.shape[0]):
#         out_arr[i] = scale[final_matrix[i]]
...
class Antialiasing:
    ...
    def __call__(self, img):
        ...
        def _convolve_frame(j):
            convolve(img[ j, :, :], self.kernel,
                     mode="constant", output=img_smooth[j, :, :])
            img_smooth[j, :, :] *= s

        futures = []
        # for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            args = (_convolve_frame,j)
