...
# from .TCIR_sampling import EqualFrequencySampler
...
class BatchGenerator:
    def __init__(self,
        ...
        if (sampler_file is None) or not os.path.isfile(sampler_file):
            ...
            # )
            self.sampler = EqualFrequencySampler(
                            sampling_bins, raw[primary_raw_var], sample_shape,
                            time_range_valid, time_range_sampling=time_range_sampling,
                            timestep_secs=self.interval_secs
                        )
    ...
    def batch(self, samples=None, batch_size=None):
        ...
        for var_name in self.used_variables:
            ...
            t = np.transpose(t0_shifted) + ts_secs * var_data["timesteps"][None, :]
            # t_relative = (t - t0[:, None]) / self.interval_secs
            t_relative = (t - np.transpose(t0)) / self.interval_secs

            # read raw data from index
            raw_data = (
                self.patch_index[raw_name](t)
...
class PatchIndex:
    ...
    def __init__(
        ...
        self.patch_times = patch_times
        # self.sample_shape = (
        #     self.patch_data.shape[1] * box_size[0],
        #     self.patch_data.shape[2] * box_size[1]
        # )
        self.sample_shape = (
            self.patch_data.shape[1],
            self.patch_data.shape[2]
        )
    ...
    def __call__(self, t):
        ...
        # 使用默认值
        i0_all = np.zeros(t.shape[0], dtype=int)
        j0_all = np.zeros(t.shape[0], dtype=int)
        ...
        fill_batch_from_raw_data(t, self.patch_data,self.patch_times, batch)
...
# 替代原来的build_batch函数
def fill_batch_from_raw_data(t, patch_data,patch_times, batch):
    # 获取原始数据
    # patch_data = raw_data['patch_data']
    # patch_times = raw_data['patch_times']

    # 遍历t矩阵的每一项
    # for batch_index, time_indices in enumerate(t):
        # 遍历t矩阵中每个时间步的索引
        # for time_index in range(time_indices.shape[0]):
        #     # 获取t矩阵中当前时间步对应的patch_times索引
        #     patch_time_index = time_indices[time_index]
    ...
    for i in range(t.shape[0]):
        for j in range(t.shape[1]):
            ...
            # 在patch_times中找到这个元素的索引
            index = np.where(patch_times == element)[0]

            # 如果找到了索引
            if index.size > 0:
                # 将patches中对应索引的元素复制到batch矩阵中
                batch[i, j] = patch_data[index[0]]


            # # 检查索引是否在patch_times的范围内
            # if 0 <= patch_time_index < patch_times.shape[0]:
            #     # 根据索引从patch_times获取对应的时间点
            #     time_point = patch_times[patch_time_index]
            #
            #     # 根据索引从patch_data获取对应的patch
            #     patch = patch_data[patch_time_index]
            #
            #     # 将patch填充到batch矩阵中
            #     # 假设time_index对应batch的第二维索引，batch_index对应第一维索引
            #     batch[batch_index, time_index] = patch
